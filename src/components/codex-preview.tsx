...
export function CodexPreview({
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="mt-[2px] space-y-[2px]">
          <PreviewRow
            ...
            onClick={(e) => {
              setScene('task')
              targetEvents('threadRows').onClick(e)
            }}
          />
          <PreviewRow
            label="完善主题预览"
            style={isHome ? rowStyle(theme.ui.threadRows, theme.accent, 'normal') : rowStyle(theme.ui.threadRows, theme.accent, 'hover')}
            className={targetClass('threadRows')}
            onClick={(e) => {
              setScene('task')
              targetEvents('threadRows').onClick(e)
      ...
      {/* 场景切换器（Segmented Control） */}
      <div className="absolute right-[8px] bottom-[8px] z-50 flex rounded-full bg-black/60 p-[1.5px] backdrop-blur-md border border-white/10 text-[5px] font-bold shadow-lg">
        <button
          onClick={(e) => { e.stopPropagation(); setScene('task'); }}
          className={cn(
            "px-[6px] py-[2.5px] rounded-full transition-all duration-300 cursor-pointer",
            scene === 'task' ? "bg-white text-zinc-950 shadow-sm" : "text-zinc-400 hover:text-white"
          )}
        >
          任务场景
        </button>
        <button
          onClick={(e) => { e.stopPropagation(); setScene('home'); }}
          className={cn(
            "px-[6px] py-[2.5px] rounded-full transition-all duration-300 cursor-pointer",
            scene === 'home' ? "bg-white text-zinc-950 shadow-sm" : "text-zinc-400 hover:text-white"
          )}
        >
          欢迎主页
        </button>
