...
| macOS | Apple Silicon, Intel | The official Codex Desktop with a valid signature |

The current release supports Windows x64 and macOS (Apple Silicon and Intel). Linux is not supported. Skin Studio verifies the official Codex installation before it performs an injection: Windows uses the Microsoft Store package, while macOS verifies the app signature and OpenAI team identifier. It does nothing when the installation cannot be found or verified.
